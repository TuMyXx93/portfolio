...
export const useSmoothTransition = (sectionIds: string[], options: UseSmoothTransitionOptions = {}) => {
  ...
  const handleScroll = useThrottle(() => {
    const sections = sectionIds.map(id => {
      ...
      return {
        id,
        visible: rect.top <= window.innerHeight * threshold && rect.bottom >= window.innerHeight * threshold
      }
    }).filter(Boolean)
